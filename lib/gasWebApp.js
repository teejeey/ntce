...
/** Same-origin API path for route handlers (browser uses relative URLs). */
export function getInternalApiUrl(path) {
  ...
  if (explicit) return `${explicit.replace(/\/$/, "")}${p}`;
...
export async function submitRegistrationViaGas(body, options = {}) {
  ...
  if (
    ...
    return {
      ok: false,
      error:
        "Full name, email, mobile number, organization, designation, and topic interest are required.",
    };
  }
